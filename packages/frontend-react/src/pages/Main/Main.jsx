    ...
    return (
        <>
            <TransitionOverlay>
                ...
                <Formik validationSchema={validationSchema} initialValues={{ check: true, priority: 2,title: '', role: '',date: '',desc: '' }} onSubmit={console.log} enableReinitialize={true}>
                    {({handleSubmit,handleChange,values,setFieldValue,touched,errors})=>(<form onSubmit={(e)=>{e.preventDefault();handleSubmit()}}>
                        ...
                        <Input label='title' name='title' onChange={handleChange} value={values.title} touched={touched.title} errors={errors.title}/>
                        <Checkbox name='check' id='check' onChange={handleChange} value={values.check}/>
